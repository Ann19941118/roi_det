...
if __name__ == "__main__":
    ...
    file_path = './data/coco/img_rotate5/'
    json_names = os.listdir(file_path)
    for json_name in json_names:
        if json_name.endswith('.json'):
            print(json_name)
            img = load_image(file_path+json_name.replace('json','jpg'))
            points,data = decode_json(file_path,json_name)
            ...
            # for image, kpoints in gen.generate(10):
            #     display_with_landmark(image, kpoints)

            for i, out in enumerate(gen.generate(5)):    
                image ,k=out
                image =image*255
                landmarks = [[int(k[i]),int(k[i+1])] for i in range(0,len(k),2)]
                print(landmarks[0][0]-landmarks[-1][0],landmarks[0][1]-landmarks[-1][1])
                cv2.imwrite('./data/coco/img_aug5/'+json_name[:-5]+'_'+str(i)+'.jpg',image) # save the augmented image
                data["imageData"]= string = base64.b64encode(cv2.imencode('.jpg', image)[1]).decode()
                data['shapes'][0]['points'] = landmarks
                with open('./data/coco/img_aug5/'+json_name[:-5]+'_'+str(i)+'.json', 'w',encoding='utf-8') as f:
                    json.dump(data,f)
