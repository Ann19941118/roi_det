...
if __name__ == "__main__":
    ...
    file_path = './data/coco/img_rotate/'
    ...
    for json_name in json_names:
        if json_name.endswith('.json'):
            ...
            for i, out in enumerate(gen.generate(20)):     
                image ,k=out
                image =image*255
                landmarks = [[int(k[i]),int(k[i+1])] for i in range(0,len(k),2)]
                cv2.imwrite('./data/coco/img_aug/'+json_name[:-5]+'_'+str(i)+'.jpg',image) # save the augmented image
                ...
                with open('./data/coco/img_aug/'+json_name[:-5]+'_'+str(i)+'.json', 'w',encoding='utf-8') as f:
                    json.dump(data,f)
